...
export function TenantDetailPage() {
  ...
  if (tenant.isLoading) return <div className="text-sm text-fg-3">Loading…</div>;
  ...
  return (
    <div>
      ...
      {archive.error && (
        <div className="text-sm text-danger-700 mb-3">{(archive.error as Error).message}</div>
      )}

      <section className="card p-5 mb-6">
        <dl className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
          <div>
            <dt className="text-xs uppercase text-fg-3">Phone</dt>
            <dd className="mt-1">{t.phone ?? '—'}</dd>
          </div>
          <div>
            <dt className="text-xs uppercase text-fg-3">Government ID</dt>
            <dd className="mt-1">
              {[t.gov_id_type, t.gov_id_number].filter(Boolean).join(' · ') || '—'}
            </dd>
          </div>
          <div>
            <dt className="text-xs uppercase text-fg-3">Emergency contact</dt>
            <dd className="mt-1">
          ...
          <div className="md:col-span-3">
            <dt className="text-xs uppercase text-fg-3">Notes</dt>
